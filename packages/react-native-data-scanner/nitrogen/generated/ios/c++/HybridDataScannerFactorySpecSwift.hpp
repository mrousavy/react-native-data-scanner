...
namespace margelo::nitro::datascanner {
  ...
  public:
    // Methods
    inline std::shared_ptr<Promise<ScannedCode>> scan(const ResolvedScanOptions& options) override {
      auto __result = _swiftPart.scan(std::forward<decltype(options)>(options));
